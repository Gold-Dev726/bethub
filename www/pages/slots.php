<!-- <script type="text/javascript" src="https://casino.int.a8r.games/public/sg.js"></script> -->
...
<script type="text/javascript">
  ...
  let BACKEND_URL = isDebug ? `https://${isDebug}` : 'https://bethub.gg:2053';
  ...
  let returnGameHtml = game => {
    let name = game.identifier.split(':');
    let prodData = producerNames.filter(x => x.id == game.producer)[0] || {name: game.producer, id: game.producer};

    return `
      <a class="slots-game" href="/slots_game/${name[0]}/${name[1].replaceAll('-', '___')}" id="game_${game.title.toLowerCase()}">
        <div class="game">
          <img onerror="this.onerror=null;this.setAttribute('src', 'https://steamuserimages-a.akamaihd.net/ugc/885384897182110030/F095539864AC9E94AE5236E04C8CA7C2725BCEFF/');" src="https://cdn.softswiss.net/i/s4/${game.provider}/${name[1]}.png" alt="" />
  ...
  let getGames = async (cb) => {
    let r = await fetch(`${BACKEND_URL}/softswiss/games`);
